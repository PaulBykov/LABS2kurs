			...
			else
			{
				if (j != 0)
				{
					...
					if (Analyse(temp, str_number, lexTable, idTable))
					{
						...
						throw ERROR_THROW_IN(123, str_number, str_position + 1);
				}
				else
				{
					if (source.text[i] == '\'')
					{
						...
						for (int c = 0; source.text[i] != '\''; c++)
						{
							if (c <= TI_STR_MAXSIZE)
							{
								...
								throw ERROR_THROW_IN(127, str_number, str_position + 1);
						}
						...
						if (source.text[i] == '\'')
						{
							...
							throw ERROR_THROW_IN(124, str_number, str_position + 1);
					}
